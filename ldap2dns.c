...
static void calc_checksum(int* num, int* sum)
{
	...
	*num = *sum = 0;
	ldaperr = ldap_search_ext_s(ldap_con,
		options.searchbase[0] ? options.searchbase : NULL,
		LDAP_SCOPE_ONELEVEL, "objectclass=DNSzone",attr_list,
		0, NULL, NULL, &options.searchtimeout, options.reclimit, &res);
